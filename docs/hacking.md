...
Repo internals for agents and humans working on the skills pack itself. `CLAUDE.md` keeps the must-not-forget rules; everything else lives here so it doesn't bloat every session's context.
...
`bin/lsdlc-linear` is a Node script that wraps Linear's GraphQL API. Skills call it for every Linear operation. Subcommands: `whoami`, `search-issues`, `list-assigned`, `get-issue`, `set-status`, `create-issue`, `add-relation`, `update-issue`, `document-upsert`. All output is JSON on stdout.
...
See `CLAUDE.md` for the critical security invariants around how the API key is loaded.
...
Run these after touching anything load-bearing:
