...
## v2.5.0 — 2026-04-10 — `/kickoff` for day-zero projects, type-specific issue overlays, and error capture as learnings

**New skill: `/kickoff`.** Run once at the start of a brand-new project. Walks the user through vision, target users (1–3 personas authored inline), success metrics, constraints, MVP, non-goals, tech-stack sketch, risks, and open questions, then writes `docs/CHARTER.md` from `templates/charter-template.md` and one file per persona under `docs/personas/`. Idempotent on re-run — if `docs/CHARTER.md` already exists, the skill asks whether to edit it, archive and start fresh, or cancel; never silent-overwrite. Hands off explicitly to `/brainstorm` for the first MVP feature without auto-invoking it.

`/kickoff` is a separate skill from `/brainstorm` rather than a Step-0 branch inside it, on purpose: kickoff runs once per project while brainstorm runs many times — sharing one skill would make every brainstorm pay context cost for a check that almost never fires. The artifacts are also different (foundational `docs/CHARTER.md` read by every later skill vs per-feature `specs/<slug>.md`), and the questions are different (vision/users/MVP/non-goals vs problem/solution/breakdown). Model/effort: opus / medium, matching `/brainstorm` — both are foundational synthesis read by every later skill.

**New templates.** `templates/charter-template.md`, `templates/persona-template.md`, and `templates/adr-template.md` for the day-zero artifacts. `templates/issue-types/{epic,story,bug,spike}.md` for type-specific overlays applied on top of the base parent / sub-issue shapes in `templates/issue-template.md`:

- `epic` extends the parent shape with Outcome / Success metrics / Personas served / Milestones / Risks
- `story` replaces the freeform AC checklist with Gherkin scenarios and adds Designs / Telemetry / Copy
- `bug` replaces Requirements with Steps to reproduce / Expected / Actual / Environment / Severity / Suspected root cause
- `spike` replaces Requirements + AC with Question / Time-box / Deliverable / Decision criteria

The base `templates/issue-template.md` sub-issue shape now also carries an explicit `## Test plan` section and a shared `## Definition of done` baseline that `/implement` walks during its Step 7 completeness check. The base `templates/spec-template.md` gains optional Personas / Success metrics / Non-goals / Risks & assumptions / Rollout sections — marked optional so non-greenfield brainstorms ignore them.

**`/brainstorm` charter-aware grounding.** New Step 1.4 reads `docs/CHARTER.md` if present and surfaces vision + relevant non-goals as grounding before the wiki-prior-art search and the duplicate check. If the topic conflicts with a non-goal, surface it explicitly via `AskUserQuestion`. In deep-design mode, structural decisions are now written as ADRs to `docs/adr/NNNN-<slug>.md` from `templates/adr-template.md` instead of being inlined in the spec — routine implementation choices stay as inline notes.

**`/create-tickets` and `/update-tickets` overlay-aware.** Step 2 of `/create-tickets` now picks a type label per ticket (`epic`, `story`, `bug`, `spike`, `chore`, or none) and Step 4a loads the matching overlay from `templates/issue-types/<type>.md` on top of the base shape. Tickets with no type label use the base shape unchanged — full backwards compatibility with legacy issues. `/update-tickets` Step 3a classifies each target by its labels and applies the same overlay during refresh, preserving overlay sections verbatim instead of flattening them back to base shape. The overlay merge rules (epic appends after `## Goal`; story replaces AC with Gherkin and substitutes the User story header; bug replaces Requirements; spike replaces Requirements + AC) live in `skills/create-tickets/SKILL.md` Step 4a so both skills stay in lockstep.

**Error capture as learnings.** New `_lsdlc_capture_error <step> <key> <insight>` shell function in `references/preamble.sh` that every skill has in scope after sourcing the preamble. It writes a `type:"error"` row to `learnings.jsonl` (per-project local state) **and** a timeline event in one call. Skills call it explicitly at known failure points right before reporting `STATUS: BLOCKED` or `STATUS: DONE_WITH_CONCERNS` — `references/completion-status.md` now requires the call as part of the status protocol, not as a nice-to-have. Wired into `/implement` (Step 3 pre-flight + Step 7e completeness gap), `/debug` (repro failure + boundary-walk stuck), `/checkpoint` (write failure), `/health` (zero quality tools detected), `/next` (Linear list failure), `/brainstorm` (search + spec-write failures), `/create-tickets` (Step 4b partial creation failure), `/update-tickets` (Step 6 partial update failure), and `/kickoff` (charter / persona write failure).

The function is intentionally call-site driven, not a global `trap ERR` — a blanket trap would fire on every harmless `grep` no-match and pollute the learnings file. Errors stay in `~/.linear-sdlc/projects/<slug>/learnings.jsonl` (local, never committed) and the per-project timeline; they explicitly do **not** go to `.linear-sdlc/wiki/log.md`, which is committed to the repo and where stack traces, file paths, and customer-environment details would leak into git history. The next session sees prior errors automatically through the 3-line learnings preview at the top of every skill run; explicit lookup is `lsdlc-learnings-search --type error`.

**Greenfield flow:** `/kickoff` → `/brainstorm` → `/create-tickets` → `/next` → `/implement`. Each step now grounds itself in the artifact the previous step produced — kickoff writes the charter, brainstorm reads it, create-tickets emits typed Linear issues that match what the spec asked for, and implement reads the per-type acceptance criteria back as the binding contract.

**No `setup` changes needed.** `setup` already globs `skills/*/SKILL.md` and symlinks each into `~/.claude/skills/`, so a fresh `./setup` run picks up `/kickoff` automatically. No new bin scripts, no schema changes, no MCP changes.
