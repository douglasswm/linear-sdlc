...
- `.claude-plugin/plugin.json` — plugin manifest. `name: linear-sdlc` becomes the invocation prefix (`/linear-sdlc:<skill>`). Declares `userConfig` (Linear API key + team ID) and references `.claude-plugin/mcp.json` for the Linear MCP server.
...
- `.claude-plugin/mcp.json` — Linear MCP server definition. Reads `LINEAR_API_KEY` from `${user_config.linear_api_key}`, which Claude Code sources from the OS keychain. **Kept inside `.claude-plugin/` rather than at the repo root** so Claude Code's project-scope `.mcp.json` scanner doesn't also load it and emit a bogus "Missing environment variables: user_config.linear_api_key" warning from `/doctor` — that scanner doesn't understand plugin userConfig interpolation.
